...
namespace Badass.Templating.DatabaseFunctions.Adapters
{
    public class SelectForDisplayDbTypeAdapter : DbTypeAdapter
    {
            ...
            get
            {
                if (_fields == null)
                {
                    ...
                    foreach (var fld in _applicationType.Fields)
                    {
                        ...
                        {
                            _fields.Add(new DbFieldAdapter(fld, this));
                        }
        ...
        private string GenerateOwnershipExpression(string currentUserIdentifier)
        {
            ...
            {
                GetDirectOwnershipExpression(currentUserIdentifier, sb, GetAliasForLinkingField(_applicationType.Fields.First(f => f.IsIdentity)));
            }
            else
            {
